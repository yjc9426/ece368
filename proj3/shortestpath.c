...
void Dijkstra(Graph g, int from, int target)
{
    ...
    while (flag[a] == 0){
	flag[a] = 1;
        for (i=0; i< g->alist[a]->d;i++){
	    weight = dist[a] + get_distance(g,a,g->alist[a]->list[i]);
	    //printf("%dweight\n",weight);
	    //printf("a%d,alist%d\n",a,g->alist[a]);

	    if (weight < dist[g->alist[a]->list[i]] && flag[g->alist[a]->list[i]] == 0){
	    	dist[g->alist[a]->list[i]] = weight;
		prev[g->alist[a]->list[i]] = a; 
		//printf("%d,%d,%d number in list\n",a,weight,prev[a]);
	    }

	}
	    //printf("end of 1 loop");
        a = 1;
	int distan = infi;
	for (i=1; i<g->v;i++){
...
//push
   ...
   // printf("j value:%d\n",j);    
    int distance = 0;
    for(k = 1;k<j;k++){
	distance += get_distance(g,s[k],s[k-1]);
